...
class SandboxConfig:
    ...
    # 禁止的 AST 节点类型
    forbidden_ast_nodes: Set[str] = field(default_factory=lambda: {
        'Import',  # 我们会单独处理导入
        'ImportFrom',
    })
...
class RestrictedBuiltins:
    ...
    ALLOWED_BUILTINS = {
        ...
        'getattr', 'hasattr', 'hash', 'hex', 'id', 'isinstance',
        'issubclass', 'iter', 'len', 'map', 'max', 'min',
        'next', 'oct', 'ord', 'pow', 'print', 'range',
        'repr', 'reversed', 'round', 'slice', 'sorted',
        'sum', 'type', 'vars', 'zip',
        ...
        'NameError', 'UnboundLocalError', 'OSError', 'IOError',
        'EOFError', 'MemoryError', 'RecursionError',
        'NotImplementedError', 'IndentationError', 'TabError',
        ...
        for node in ast.walk(tree):
            ...
            # 检查危险的属性访问
                ...
                if node.attr.startswith('_'):
                    ...
                    allowed_dunders = {'__name__', '__doc__', '__class__', '__dict__',
                                      '__len__', '__iter__', '__next__', '__str__',
                                      '__repr__', '__init__', '__call__', '__getitem__',
                                      ...
                                      '__abs__', '__enter__', '__exit__', '__aenter__', '__aexit__',
...
class CodeSandbox:
    ...
    def execute(self, code: str, validate: bool = True, use_subprocess: bool = False) -> ExecutionResult:
        """
        在沙箱中执行代码
    ...
    async def execute_async(self, code: str, validate: bool = True) -> ExecutionResult:
        """
        异步执行代码
        ...
        return await loop.run_in_executor(
            None,
            lambda: self.execute(code, validate)
        )
    
    def execute_with_async_tools(self, code: str, validate: bool = True) -> ExecutionResult:
        """
        执行包含异步工具调用的代码
